...
- Add a debug-only solve warning that names each output returned with an empty `InnerTree` (and flags whether all or only some outputs were empty), so a definition that computed nothing is diagnosable. Gated on `config.debug`; empty outputs can be legitimate so this never fails the solve.
